...
describe('Server', () => {
    ...
    describe('POST to /', () => {
        it('should return a status code of 201', async () => {
            const response = await request(server).post('/').send({id: 4, name: 'Luis', email: 'luis@test.com'});
            
            expect(response.status).toEqual(201); 
        })

        it('should return a status code of 500', async () => {
            const response = await request(server).post('/').send({id: 4, name: 'Luis',})//no name 
        ...
        it('should return posted data in the response body', async () => {
            ...
            const response = await request(server).post('/').send(expectedBody)
